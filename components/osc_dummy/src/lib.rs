...
#[used]
static mut DUMMY: MaybeUninit<Dummy> = MaybeUninit::uninit();
...
pub extern "C" fn unit_init(desc: *const unit_runtime_desc_t) -> i8 {
    unsafe {
        DUMMY.as_mut_ptr().write(Dummy::new());
        DUMMY.assume_init_mut().init(desc)
    }
}
...
pub extern "C" fn unit_teardown() {
    unsafe {
        DUMMY.assume_init_mut().teardown();
    }
}
...
pub extern "C" fn unit_render(input: *const f32, output: *mut f32, frames: u32) {
    unsafe {
        DUMMY.assume_init_mut().process(input, output, frames);
    }
}
...
pub extern "C" fn unit_get_param_value(index: u8) -> i32 {
    unsafe {
        DUMMY.assume_init_mut().get_parameter_value(index)
    }
}
...
pub extern "C" fn unit_get_param_str_value(index: u8, value: i32) -> *const core::ffi::c_char {
    unsafe {
        DUMMY.assume_init_mut().get_parameter_str_value(index, value)
    }
}
...
pub extern "C" fn unit_set_param_value(index: u8, value: i32) {
    unsafe {
        DUMMY.assume_init_mut().set_parameter(index, value);
    }
}
...
pub extern "C" fn unit_note_on(note: u8, velo: u8) {
    unsafe {
        DUMMY.assume_init_mut().note_on(note, velo);
    }
}
...
pub extern "C" fn unit_note_off(note: u8) {
    unsafe {
        DUMMY.assume_init_mut().note_off(note);
    }
}
...
pub extern "C" fn unit_all_note_off() {
    unsafe {
        DUMMY.assume_init_mut().all_note_off();
    }
}
...
pub extern "C" fn unit_pitch_bend(bend: u8) {
    unsafe {
        DUMMY.assume_init_mut().pitch_bend(bend);
    }
}
...
pub extern "C" fn unit_channel_pressure(press: u8) {
    unsafe {
        DUMMY.assume_init_mut().channel_pressure(press);
    }
}
...
pub extern "C" fn unit_aftertouch(note: u8, press: u8) {
    unsafe {
        DUMMY.assume_init_mut().after_touch(note, press);
    }
}
